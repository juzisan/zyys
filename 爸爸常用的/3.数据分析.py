...
#在循环用exec引用
    ...
    Ofen_renshu_zutifenxi = yilie[yilie == 0].count()
...
#在循环用exec引用
    ...
    zixing_dict = {'本题满分':'manfen_zhi',
                    ...
                    '0分人数':'Ofen_renshu_zutifenxi',
                    '0分率':'round(Ofen_renshu_zutifenxi/zong_renshu*100,1)',
    ...
    del manfen_renshu,jige_renshu_zutifenxi,Ofen_renshu_zutifenxi#为了不显示错误
...
def main():
    ...
    names = list(filter(lambda x : re.match('小分表.*.xlsx', x),names))#[i for i in names if re.match('小分表.*.xlsx', i)]
