...
def duo(xlsx_nam):
    ...
    ws = wb.create_sheet(xlsx_nam.split('.')[0])#创建工作簿名
    ...
    #print (dffff.tail())
    rows = dataframe_to_rows(biao_or)#pandas的dataframe转openpyxl的格式
...
def main():
    ...
    #数据区
    names = '签到.xlsx'
    biao_or=pd.read_excel(names,sheet_name=None,header=2)#读多个sheet，忽略前两行标题
    sheet_name = list(biao_or.keys())#获取所有sheet名称
    biao_hebing = []
    for i in sheet_name:
        biao=biao_or[i].dropna(axis=1,how='all')#删除所有空列
        del biao['序号']
    #biao=biao.reset_index()
        biao=biao.set_index("姓名")#用列替换索引
        #biao_jia = yuef(biao)
        if not biao.empty:#空表
            biao_hebing.append(biao)
    biao_hebing = pd.concat(biao_hebing,axis=1,sort=False)#默认重新索引
    print (biao_hebing)
    print (biao_hebing.count())
